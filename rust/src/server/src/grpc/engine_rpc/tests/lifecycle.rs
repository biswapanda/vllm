...
async fn health_reports_ready_without_probe() {
    ...
    let response = client.health(pb::HealthRequest::default()).await.expect("health").into_inner();
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
#[serial]
async fn drain_is_private_to_engine_rpc_admission() {
    let (mut client, server_task, _engine_task) =
        engine_rpc_test_server(&[0x00, 0x00], default_stream_output_specs()).await;
    ...
    let status = client
        ...
        .await
        .expect_err("draining engine RPC must reject new requests");
