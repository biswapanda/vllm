...
async fn lifecycle_rejects_disabled_engine() {
    ...
    let mut request = base_request();
    request.lora_name = "adapter-a".to_string();
    let error = engine_rpc.generate(request).await.unwrap_err();
    assert_eq!(error.code(), tonic::Code::FailedPrecondition);
    server_task.abort();
    drop(engine_task);
}
...
async fn lifecycle_load_timeout_marks_registry_inconsistent() {
    ...
    let health = engine_rpc.health(pb::HealthRequest::default()).await.unwrap().into_inner();
    assert_eq!(health.state, pb::HealthState::NotReady as i32);
    assert!(health.checks.iter().any(|check| check.name == "lora"));
    let mut request = base_request();
    request.lora_name = "adapter-a".to_string();
    assert_eq!(
