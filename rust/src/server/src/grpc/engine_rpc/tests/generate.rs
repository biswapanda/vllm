...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
#[serial]
async fn non_streaming_request_returns_one_text_event_and_terminal_usage() {
    let (mut client, server_task, engine_task) =
        engine_rpc_test_server(&[0x00, 0x00], default_stream_output_specs()).await;
    let mut request = base_request();
    request.stream = false;
    let responses: Vec<pb::GenerateResponse> = client
        .generate(request)
        .await
        .expect("generate")
        .into_inner()
    ...
    assert_eq!(
        responses
            ...
            .filter(|response| matches!(
                response.event,
                Some(pb::generate_response::Event::Token(_))
            ))
            .count(),
        1
    );
    assert!(matches!(
        responses.last().and_then(|response| response.event.as_ref()),
        Some(pb::generate_response::Event::Finished(_))
    ));
    assert!(responses.last().and_then(|response| response.usage.as_ref()).is_some());
    engine_task.await.expect("mock engine task");
    server_task.abort();
}
