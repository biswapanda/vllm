...
impl EngineCoreClient {
    ...
    /// result per engine. The outer error is reserved for failures that happen
    ...
    /// Call one utility method on the connected engine with this logical
    /// data-parallel rank.
    pub async fn call_utility_on_engine<T, A>(
        &self,
        ...
        A: serde::Serialize + std::fmt::Debug,
    {
        let engine = self
            .engines
            .iter()
            .find(|engine| engine.ready_response.data_parallel_rank == data_parallel_rank)
        ...
        call_guard.track(call_id);
        let request =
            match EngineCoreUtilityRequest::new(self.config.client_index, call_id, method, &args) {
                Ok(request) => request,
    ...
    where
        ...
        A: serde::Serialize + std::fmt::Debug,
    {
        self.call_utility_per_engine(method, args).await?.into_iter().collect()
    }
