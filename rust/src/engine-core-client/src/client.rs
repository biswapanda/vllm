...
impl EngineCoreClient {
    ...
    ///
    /// Topology, role, and scheduler limits are validated as uniform during
    /// startup. Per-engine fields such as `data_parallel_rank` should be read
    /// through [`ready_responses`](Self::ready_responses).
